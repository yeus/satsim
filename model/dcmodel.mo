package dcmodel
  model DCtoDCModel "DC/DC Converter Model nach Torrey, D., & Selamogullari, U. (2002). A Behavioral Model for DC-DC Converters using Modelica. modelica.org"
    ...
    Modelica.Electrical.Analog.Basic.Resistor Rhk annotation(Placement(visible = true, transformation(origin = {-47.5463,-2.79786}, extent = {{-12,12},{12,-12}}, rotation = -90)));
    Modelica.Electrical.Analog.Sensors.VoltageSensor Volt annotation(Placement(visible = true, transformation(origin = {62.9587,-6.90797}, extent = {{-12,12},{12,-12}}, rotation = -90)));
    ...
    SignalCurrent SignalCurrent1 annotation(Placement(visible = true, transformation(origin = {30.188,-5.62026}, extent = {{12,-12},{-12,12}}, rotation = -270)));
    Modelica.Electrical.Analog.Basic.Capacitor Cout(C = 0.00005) annotation(Placement(visible = true, transformation(origin = {11.0546,-6.76714}, extent = {{-12,12},{12,-12}}, rotation = -90)));
  ...
  equation
    ...
    connect(Volt.n,n1) annotation(Line(points = {{62.9587,-18.908},{63.0835,-18.908},{63.0835,-55.7992},{100.829,-55.7992}}));
    connect(Volt.p,Cout.p) annotation(Line(points = {{62.9587,5.09203},{74.116,5.09203},{74.116,5.23286},{11.0546,5.23286}}));
    connect(Cout.n,Volt.n) annotation(Line(points = {{11.0546,-18.7671},{23.1966,-18.7671},{23.1966,-18.908},{62.9587,-18.908}}));
    connect(Dout.p,Cout.n) annotation(Line(points = {{95.9451,-19.1191},{76.662,-19.1191},{76.662,-18.7671},{11.0546,-18.7671}}));
    connect(Dout.n,Cout.p) annotation(Line(points = {{95.9451,4.88093},{89.6747,4.88093},{89.6747,5.23286},{11.0546,5.23286}}));
    ...
    connect(SignalCurrent1.p,Dout.p) annotation(Line(points = {{30.188,-17.6203},{10.4668,-17.6203},{10.4668,-19.1191},{95.9451,-19.1191}}));
    connect(Dout.n,SignalCurrent1.n) annotation(Line(points = {{95.9451,4.88093},{95.6153,4.88093},{95.6153,6.37974},{30.188,6.37974}}));
    connect(Feedback1.u2,Volt.v) annotation(Line(points = {{-52.1354,69.6469},{-52.1354,39.3211},{51.768,39.3211},{51.768,-6.90797},{50.9587,-6.90797}}));
    connect(Volt.p,Amp.p) annotation(Line(points = {{62.9587,5.09203},{57.1429,5.09203},{57.1429,13.0863},{82.7203,13.0863}}));
